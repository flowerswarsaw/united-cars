...
export function RegionSelect({
  ...
  React.useEffect(() => {
    if (value && !selectedRegion && regions && regions.length > 0) {
      setIsCustomMode(true);
    } else if (!value || selectedRegion) {
      setIsCustomMode(false);
    }
  }, [value, selectedRegion, regions]);
  ...
  if (!regions || regions.length === 0 || isCustomMode) {
    return (
      <div className="relative">
        ...
        />
        {allowCustom && regions && regions.length > 0 && (
          <button
            type="button"
